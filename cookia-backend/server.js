...
const PROMPT_STEPS = `You are a chef. Give step-by-step instructions for this recipe.
...
app.post("/api/recipe-steps", async (req, res) => {
  ...
  const start = Date.now();
  const messages = [
    { role: "system", content: PROMPT_STEPS },
    { role: "user", content: `Recipe: ${name}. Ingredients: ${ingredients?.join(", ")}` },
  ];
